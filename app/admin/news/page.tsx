...
export default function AdminNewsPage() {
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // ตรวจสอบขนาดไฟล์ (ไม่ควรเกิน 2MB เพื่อความเร็ว)
    if (file.size > 2 * 1024 * 1024) {
      alert('ไฟล์มีขนาดใหญ่เกินไป (จำกัด 2MB)');
      return;
    }
...
// 📍 จุดสำคัญ: ส่งชื่อ bucket 'news' ไปด้วย 
  // API จะอ่านค่านี้จาก (formData.get('bucket') as string)
 ...
 async function handleDelete(item: NewsItem) {
    if (!confirm(`ยืนยันการลบบทความ: ${item.title}?`)) return;

    try {
      // 1. ลบรูปใน Storage (ใช้ API ตัวเดียวกับที่อัปโหลด)
      ...
      // 2. ลบบทความใน Database
      const res = await fetch(`/api/news/${item.id}`, { method: 'DELETE' });

      if (res.ok) {
        // 3. 🚩 จุดสำคัญ: สั่งให้หน้าจออัปเดตโดยการกรองตัวที่ลบออก
        setItems(prev => prev.filter((n) => n.id !== item.id));
        alert('ลบเรียบร้อยแล้ว');
      } else {
        alert('ลบจากฐานข้อมูลไม่สำเร็จ');
      }
    } catch (error) {
                ...
                <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button onClick={() => handleDelete(item)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              ...
              {/* 📍 ส่วนจัดการรูปภาพบทความ */}
              <div>
                <label className={labelClass}>รูปภาพบทความ</label>
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <label className="shrink-0">
                      <div className={`cursor-pointer px-4 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-all ${uploading ? 'bg-gray-100 text-gray-400' : 'bg-slate-900 text-white hover:bg-slate-800'}`}>
                        {uploading ? <Loader2 size={16} className="animate-spin" /> : <Upload size={16} />}
                        {uploading ? 'กำลังส่ง...' : 'อัปโหลด'}
                      </div>
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                <label htmlFor="isPublished" className="text-sm text-gray-700 font-medium">เผยแพร่บทความนี้สู่สาธารณะ</label>
              </div>
            </div>
            <div className="flex justify-end gap-3 p-6 border-t sticky bottom-0 bg-white">
              <button onClick={closeModal} className="px-5 py-2.5 border border-gray-300 text-gray-700 rounded-xl text-sm hover:bg-gray-50 transition-colors">ยกเลิก</button>
              <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 bg-[#f4511e] text-white font-semibold px-6 py-2.5 rounded-xl hover:bg-[#d43e0e] disabled:opacity-60 text-sm shadow-lg shadow-orange-500/20">
